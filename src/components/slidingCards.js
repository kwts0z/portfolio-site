...
const SlidingCards = ({ icon, title, text, rightText }) => {
  ...
  const moveLeft = useTransform(leftSpring, [0, 1], ["0%", "-40%"]);
  const moveRight = useTransform(rightSpring, [0, 1], ["0%", "40%"]);

  return (
    <div ref={ref} className="relative flex flex-row justify-center items-center w-full h-60">
      <motion.div
        style={{ translateX: moveLeft }}
        className='relative flex flex-row items-center pl-10 p-5 space-x-2 bg-accent-v2 rounded-3xl z-10 w-2/5 h-full '
      >
      ...
      <motion.div
        ...
        className='absolute bg-secondary flex justify-center ml-52 items-center rounded-se-3xl rounded-ee-3xl text-3xl z-20 w-2/6 h-full'
      >
